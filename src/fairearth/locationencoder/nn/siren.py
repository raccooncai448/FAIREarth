...
#     def __init__(self, w0 = 1.):
#         super().__init__()
#         self.w0 = nn.Parameter(torch.tensor(w0, dtype=torch.float))
#         self.s0 = nn.Parameter(torch.tensor(0.5, dtype=torch.float))  # learnable s0
    
#     def forward(self, x):
#         # Cast parameters to input dtype if needed
#         w0 = self.w0.to(dtype=x.dtype)
#         s0 = self.s0.to(dtype=x.dtype)
        
#         osc = torch.exp(1j * w0 * x)
#         envelope = torch.exp(-torch.abs(s0 * x)**2)
#         return torch.real(osc * envelope)
...
#         # Change dtype based on whether it's first layer
#         self.dtype = torch.float if is_first else torch.cfloat
        
#         weight = torch.zeros(dim_out, dim_in, dtype=self.dtype)
#         bias = torch.zeros(dim_out, dtype=self.dtype) if use_bias else None
#         self.init_(weight, bias, c = c, w0 = w0)
...
#     def forward(self, x):
#         # Cast input to layer's dtype if needed
...
#         if self.dropout:
#             if out.is_complex():
#                 real = F.dropout(out.real, training=self.training)
#                 imag = F.dropout(out.imag, training=self.training)
...
#         super().__init__()
#         self.num_layers = num_layers
#         # Adjust hidden dimension for complex numbers
#         self.dim_hidden = int(dim_hidden/np.sqrt(2))  # Similar to the previous example
#         self.degreeinput = degreeinput

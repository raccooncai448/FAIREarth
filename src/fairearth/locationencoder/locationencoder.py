...
def get_positional_encoding(name, hparams=None):
    ...
    if name == "direct":
        ...
        if "harmonics_calculation" in hparams.keys() and hparams['harmonics_calculation'] == "discretized":
            ...
            return PE.SphericalHarmonics(legendre_polys=hparams['embedding']['legendre'],
                                         harmonics_calculation=hparams['harmonics_calculation'],
                                         time=hparams['data']['time_idx'])
    elif name == "theory":
        ...
        return PE.GridAndSphere(
                       frequency_num=hparams['embedding']['frequency_num'],
                       name=name)
    else:
...
def get_neural_network(name, input_dim, hparams=None):
    if name == "linear":
        ...
        # FIX THIS SOMEHOW TO MAKE NETWORK SIZE ADJUSTABLE
        return NN.SirenNet(
                dim_in=input_dim,
                dim_hidden=hparams['architecture']['dim_hidden'],
                num_layers=hparams['architecture']['num_layers'],
                dim_out=hparams['dim_out'],
...
class LocationEncoder(pl.LightningModule):
    def __init__(self, positional_encoding_name, neural_network_name, hparams):
        ...
        #import pdb
        #pdb.set_trace()
        self.learning_rate = hparams["training"]["lr"]
        self.weight_decay = hparams["training"]["wd"]

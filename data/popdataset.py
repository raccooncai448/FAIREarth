...
def get_data_points(N=5000, seed=0, grid=False):
    DATA_DIR = "datasets/test_datasets/binned_pop_map"
    popdata = pd.read_csv(DATA_DIR + '.csv', index_col=0)

    gridData = np.array(popdata.values).reshape(1800, 3600)
    #gridData = (gridData - np.mean(gridData)) / np.std(gridData)
    #gridData -= np.min(gridData)
...
def get_data(N=5000, seed=0, grid=False):
    (lons, lats), values = get_data_points(N, seed, grid=grid)
...
class PopDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        self.train_ds = TensorDataset(*get_data(self.num_samples, seed=self.train_seed))
        self.valid_ds = TensorDataset(*get_data(int(self.num_samples / 5), seed=self.valid_seed))
    ...
    def predict_dataloader(self):
        return DataLoader(self.evalu_ds, batch_size=40000, shuffle=False, pin_memory=self.pin_memory)

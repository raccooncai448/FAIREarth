...
def get_data_points(N=5000, seed=0, cache=True, sphericaluniform=True, grid=False, latitude_sampling=False):
    if latitude_sampling:
        cachefilename = DATA_DIR + f"/landoceansdataset_latitudes{seed}.geojson"
    elif grid:
    ...
    if os.path.exists(cachefilename) and cache:
        print(f"reading dataset from {cachefilename}")
        ...
        if latitude_sampling:
            ...
            for lat in np.arange(-90, 91, 10):
                sampled_lons = rng.uniform(-180, 180, size=100)
                lats.extend([lat] * 100)
                lons.extend(sampled_lons)
            
            lats = np.array(lats)
            lons = np.array(lons)
        elif grid:
            gridsize = 1e5
            x, y = np.meshgrid(np.arange(-17367530.45, 17367530.45, step=gridsize), 
                             np.arange(-7324184.56, 7324184.56, step=gridsize))
        ...
        points = gpd.GeoDataFrame([], 
                                geometry=[Point(lon, lat) for lat, lon in zip(lons, lats)], 
                                crs=4326)
...
def get_data(N=5000, seed=0, grid=False, latitude_sampling=False):
    points = get_data_points(N, seed, grid=grid, latitude_sampling=latitude_sampling)
...
class LandOceanDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        self.evalu_ds = TensorDataset(*get_data(self.num_samples, seed=1, latitude_sampling=True))
    ...
    def test_dataloader(self):
        return DataLoader(self.valid_ds, batch_size=100, shuffle=False)

    def predict_dataloader(self):
        return DataLoader(self.evalu_ds, batch_size=100, shuffle=False)
